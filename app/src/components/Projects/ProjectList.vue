  ...
  <b-container>
    ...
    <template v-if="items.length">
      <table class="table table-striped" id="items">
        <thead>
          <tr>
            <th scope="col" v-t="'project.key.label'" />
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td>
              <router-link :to="{ name: 'ProjectEdit', params: { key: item.key } }">{{ item.key }}</router-link>
            </td>
            <td v-text="item.name" />
...
export default {
  ...
  computed: {
    ...
    sortOptions() {
      return Object.entries(this.$t('projects.sort'))
        .map(([value, text]) => ({
          text,
          value
        }))
  ...
  methods: {
    async refresh(params = null) {
      if (!this.loading) {
        this.loading = true
        try {
          const { data } = await getProjects(params ?? this.params)
